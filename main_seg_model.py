...
from train_fcn import train_fn_seg, valid_fn_seg
...
def train_loop(folds, fold):
    ...
    model = smp.Unet(CFG.model_name, classes=1, aux_params=aux_params)
    ...
    for epoch in range(CFG.epochs):
        ...
        # eval
        avg_val_loss, preds = valid_fn_seg(valid_loader, model, criterion["seg"], device)
        ...
        if avg_val_loss < best_loss or score > best_score:
            ...
            torch.save({"model": model.state_dict(), "preds": preds}, f"{CFG.model_name}_fold{fold}_S1_best.pth")
        else:
            update_count += 1
    ...
    check_point = torch.load(f"{CFG.model_name}_fold{fold}_S1_best.pth")
...
if __name__ == "__main__":
    ...
    if not os.path.exists("./"):
        os.makedirs("./")
    LOGGER = init_logger(f"{CFG.model_name}-S1-fold{CFG.trn_fold[0]}.log")
