...
from train_fcn import train_auc, valid_auc
from utils import get_score, init_logger
from losses import FocalLoss, DeepAUC
from ranczr_models import CustomResNet200D, EffNetWLF, CustomAttention
...
def train_loop(folds, fold):
    ...
    if CFG.resume:
        checkpoint = torch.load(f"{CFG.resume_path}/{CFG.model_name}-fold{fold}.pth", map_location='cpu')
    ...
    # for param in model.feature[-1].parameters():
    #     param.requires_grad = True
    ...
    # scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=pg_lr[1:], epochs=CFG.epochs, 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RANZCR-ft")
    parser.add_argument("--data", default="/home/jun/project/data/ranzcr-clip-catheter-line-classification",
    ...
    class CFG:
        ...
        resume = True
        resume_path = "results/ranczr_arch"
        size = 640
        # epochs = 25
        # lr = 0.00003
        epochs = 20
    ...
    augmentation = [
            a_transform.RandomResizedCrop(CFG.size, CFG.size, scale=(0.95, 1.0), p=1),
            ...
            # a_transform.RandomCrop(CFG.size, CFG.size, p=1),
    ...
    # valid_transform = a_transform.Compose([
    #         a_transform.SmallestMaxSize(CFG.size),
    #         a_transform.CenterCrop(CFG.size, CFG.size, p=1), 
    #         normalize,
    #         ToTensorV2()
    #     ], p=1.0)
    ...
    # main(folds)
    LOGGER = init_logger(f"{CFG.model_name}-ft.log")
